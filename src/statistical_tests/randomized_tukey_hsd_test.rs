...
pub struct RandomizedTukeyHsdTest {
    ...
    two_way_anova: TwoWayAnovaWithoutReplication,
}
...
impl RandomizedTukeyHsdTest {
    ...
    pub fn effect_sizes(&self) -> Vec<Vec<f64>> {
        let system_means = self.two_way_anova.system_means();
        let residual_stddev = self.two_way_anova.residual_variance().sqrt();
...
impl RandomizedTukeyHsdTester {
        ...
        S: AsRef<[f64]>,
    {
        ...
        let two_way_anova =
            TwoWayAnovaWithoutReplication::from_tupled_samples(samples.iter(), self.n_systems)?;
...
mod tests {
    ...
    #[test]
    fn test_randomized_tukey_hsd_test_sakai_book_15() {
        // From Table 5.1 in Sakai's book, "情報アクセス評価方法論".
        let a = vec![
            0.70, 0.30, 0.20, 0.60, 0.40, 0.40, 0.00, 0.70, 0.10, 0.30, //
            0.50, 0.40, 0.00, 0.60, 0.50, 0.30, 0.10, 0.50, 0.20, 0.10,
        ...
        let tupled_samples = a
            ...
            .zip(c.iter())
            .map(|((&a, &b), &c)| [a, b, c]);
        let stat = RandomizedTukeyHsdTest::from_tupled_samples(tupled_samples, 3).unwrap();
